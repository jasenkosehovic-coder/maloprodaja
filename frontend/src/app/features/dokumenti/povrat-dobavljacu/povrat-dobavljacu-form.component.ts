...
@Component({
  selector: 'app-povrat-dobavljacu-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './povrat-dobavljacu-form.component.html',
  styleUrl: './povrat-dobavljacu-form.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class PovratDobavljacuFormComponent implements OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<PovratDobavljacuFormComponent>);
  ...
  private ucitajTipPD(): void {
    this.prometService.getTipoviDokumenata()
      ...
      .subscribe(tipovi => {
        const tip = tipovi.find(t => t.kod === 'PD');
  ...
  onSubmit(): void {
    ...
    if (!tipId) {
      this.notification.error('Tip dokumenta PD nije pronađen.');
      return;
    }
    ...
    this.prometService.create(dto)
      .pipe(
        ...
        catchError(err => {
          this.notification.error('Greška pri kreiranju povrata dobavljaču.');
          console.error(err);
          return EMPTY;
        }),
        takeUntilDestroyed(this.destroyRef)
      ...
      .subscribe(result => {
        this.notification.success('Povrat dobavljaču je uspješno kreiran.');
        this.dialogRef.close(true);
        void this.router.navigate(['/dokumenti/povrat-dobavljacu', result.id]);
      });
  }
