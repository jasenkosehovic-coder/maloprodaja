...
import { MedjuskladisnicaFormComponent } from './medjuskladisnica-form.component';

@Component({
  selector: 'app-medjuskladisnica-list',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    MatFormFieldModule,
  ...
  templateUrl: './medjuskladisnica-list.component.html',
  styleUrl: './medjuskladisnica-list.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MedjuskladisnicaListComponent implements OnInit {
  ...
  readonly fields: CrudFieldConfig[] = [
    ...
    { key: 'poslovnicaNaziv', label: 'Izvor (pošiljalac)', type: 'text', readOnly: true },
    {
  ...
  readonly actions: CrudActionsConfig = { add: true, edit: false, delete: false, export: true };

  readonly filterableColumns = ['status', 'datum', 'poslovnicaNaziv', 'brojDokumenta'];
  ...
  ucitajPodatke(): void {
    ...
    this.prometService.list({ tipKod: 'MSI' })
      .pipe(
        finalize(() => {
        ...
        catchError(err => {
          this.notification.error('Greška pri učitavanju međuskladišnica.');
          console.error(err);
          return EMPTY;
        }),
  ...
  onAddClick(): void {
    const dialogRef = this.dialog.open(MedjuskladisnicaFormComponent, {
      width: '960px',
      maxWidth: '95vw',
      disableClose: true,
    });
