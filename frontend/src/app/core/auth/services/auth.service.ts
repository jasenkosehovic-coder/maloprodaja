...
export class AuthService {
  ...
  private persistSession(response: LoginResponse): void {
    ...
    sessionStorage.setItem(KORISNIK_KEY, JSON.stringify(response.korisnik));
  ...
  private clearSession(): void {
    ...
    sessionStorage.removeItem(KORISNIK_KEY);
  ...
  private loadKorisnikFromStorage(): KorisnikInfo | null {
    const raw = sessionStorage.getItem(KORISNIK_KEY);
    if (!raw) return null;
    try {
