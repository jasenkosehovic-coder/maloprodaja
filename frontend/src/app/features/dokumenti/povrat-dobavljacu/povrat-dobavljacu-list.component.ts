...
import { PovratDobavljacuFormComponent } from './povrat-dobavljacu-form.component';

@Component({
  selector: 'app-povrat-dobavljacu-list',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    MatFormFieldModule,
  ...
  templateUrl: './povrat-dobavljacu-list.component.html',
  styleUrl: './povrat-dobavljacu-list.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class PovratDobavljacuListComponent implements OnInit {
  ...
  ucitajPodatke(): void {
    ...
    this.prometService.list({ tipKod: 'PD' })
      .pipe(
        finalize(() => {
        ...
        catchError(err => {
          this.notification.error('Greška pri učitavanju povrata dobavljaču.');
          console.error(err);
          return EMPTY;
        }),
  ...
  onAddClick(): void {
    const dialogRef = this.dialog.open(PovratDobavljacuFormComponent, {
      width: '900px',
      maxWidth: '95vw',
      disableClose: true,
    });
  ...
  onEditClick(row: DokumentListItem): void {
    void this.router.navigate(['/dokumenti/povrat-dobavljacu', row.id]);
  }
